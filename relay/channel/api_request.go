...
func processHeaderOverride(info *common.RelayInfo, c *gin.Context) (map[string]string, error) {
	...
	if len(headerOverrideSource) == 0 {
		logger.LogDebug(c, "请求头覆盖配置: channel_id=%d, channel_type=%d, headers=空", channelId, channelType)
	} else {
		logger.LogDebug(c, "请求头覆盖配置: channel_id=%d, channel_type=%d, headers=%s", channelId, channelType, common2.GetJsonString(maskHeaderOverrideForLog(headerOverrideSource, info.ApiKey)))
	}
